...
def combine(out_dir: Path, summaries: list[dict]) -> None:
...
def main() -> None:
    ...
    meta, refs = load_maps(Path(args.clips), Path(args.transcripts))
    workers = max(1, min(args.workers, len(shards)))
    print(f"[audit] shards={len(shards)} workers={workers} out={out_dir}", flush=True)
    print("[audit] CPU/data-only path: no torch, no Whisper, no model weights.", flush=True)

    t0 = time.time()
    summaries: list[dict] = []
    tasks = [(str(shard), str(out_dir), meta, refs, args.max_clips_per_shard) for shard in shards]
